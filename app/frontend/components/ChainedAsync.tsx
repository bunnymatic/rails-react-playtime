...
const logit = (msg:unknown) => { console.log(`[${(new Date()).getTime()}]`, msg) }

const doMutation = ({waitTime, requestId}: {waitTime:number, requestId: string}) => (    
...
const useAutoSave = ({asyncRequests, enqueue}) => {
    ...
    async (val:{waitTime:number, requestId: string}) => {
        ...
        const currentAutoSave = maybePreviousAutoSave.then(async (previousResult) => {
...
const ChainedAsyncUpdater = ({asyncRequests, enqueue}): FC<ChainedAsyncUpdaterProps> => {
    ...
    const onClick = async () => {
        logit('clicked')
        return handleClick().then((r) => logit(`${JSON.stringify(r)} click resolved `))
    }

    return (<div>
        <div>
            <h2>Queue Size: #{asyncRequests.length}</h2>
            { entries.map(e => (<div key={e.requestId}>
...
export const ChainedAsync = (): FC => {
    ...
    const [asyncRequests, setAsyncRequests] = useState<() => Promise<Array[number]>>([])
    const enqueue = (request) => {
      setAsyncRequests([...asyncRequests, request])
      return request;
    }

    return <ChainedAsyncUpdater asyncRequests={asyncRequests} enqueue={enqueue} />
}